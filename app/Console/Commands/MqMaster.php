...
class MqMaster extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:MqMaster {arg1}';
    ...
    public function handle()
    {
        try{
            ...
            if($queueName != 'all'){
                ...
                $info = $model->where('process_name',$queueName)->where('type',0)->get()->toArray();
                if(empty($info)){
                    die("查找不到MQ:".$queueName);
                }
                $cron_groups = $info;
            }else{
            ...
            if(is_array($cron_groups) && !empty($cron_groups)){
                ...
                $group_id         = $group['id'] ?? '';
                $max_process_num  = $group['max_process_num'] ?? $group['max_process_num'];
                $current_task_num = $this->_getTasknumsByGroup($group_id);
                $total_task_num   = $this->_getTotalTaskNums();
                $queue_code       = $group['queue_name'] ?? '';
                $exchange = $group['exchange_name'] ?? '';
                $route = $group['router_key'] ?? '';
                $sparkPath = str_replace(['app\\','app/'],'',app_path().DIRECTORY_SEPARATOR);
                $command = '/usr/bin/php '.$sparkPath.'artisan command:MqProcess';
                $reday_msg_count = $amqp->getMessageCount($queue_code);
                echo $queue_code.'：队列现有总数'.$reday_msg_count.PHP_EOL;
               ...
               //如果总的进程数没超过系统最大限定值并且当前任务数<最大可执行数
               if(($total_task_num < $allow_all) &&  ($current_task_num < $max_process_num)){
                for($i =0; $i < $max_process_num;$i++){
                    ...
                    $child = new \Swoole\Process(function (\Swoole\Process $process) use ($exchange,$queue_code,$route,$command,$sparkPath,$group_id) {
                        $process->exec('/usr/bin/php',[$sparkPath.'artisan','command:MqProcess',$queue_code,$group_id]);
                    });

                    $child_id = $child->start();
                    $redis->set($this->_getKey('cron_group_key_' . $child_id, $group_id), $group_id);
